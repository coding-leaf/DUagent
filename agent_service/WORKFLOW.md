...
- `2026-06-15` `evaluation summary-only LLM enrichment + profile direction enum`：**通过**。按 Backend 新画像需求，`evaluation/generate` 新增可选上下文 `student_profile`、`profile_context`、`kg_context`、`learning_activity`，prompt 要求 LLM 基于 KG、个人资料、规则画像和学习行为按“学习范围 / 当前掌握 / 学习行为 / 下一步建议”生成 `summary_text`。同时收紧 enrichment 边界：LLM 只允许替换 `summary_text`，`progress_table`、`mastery_table`、`resource_usage_table` 保持 Backend/规则结果，不再由 LLM 改写。验证：`../.venv/bin/python -m pytest tests/test_openapi_alignment.py tests/test_evaluation_agent.py -q -p no:cacheprovider` **40 passed**；Agent API OpenAPI JSON 校验通过；相关 Python 文件 compile 通过。契约：同步更新根目录 `docs/20-agent-api/*`。
- `2026-06-14` `tutoring Guard 放宽 focus_points 硬拦截`：**通过**。根因：`tutoring_response_critic._is_relevant()` 把 `strategy.focus_points` 直接纳入硬相关术语；当用户没有画像薄弱点且检索上下文没有结构化 knowledge_points 时，Strategy 会把用户原话截断成 focus point（如 `我想复习C语言2026 01的薄弱点，请`），模型正常 RAG 回答不会逐字包含该片段，导致 ReAct 有效答案被误判 `off_topic` 并丢弃，前端只收到规则兜底和 `review empty_response`。修复：Guard 硬相关术语改为画像薄弱/掌握点、检索结构化知识点、KG 节点名；不再把自由文本 focus point 作为硬命中条件，空响应和明确结构化术语不匹配仍保持拦截。验证：先新增红灯用例复现 `off_topic` 误杀；修复后 `../.venv/bin/python -m pytest tests/test_tutoring_response_critic.py tests/test_tutoring_fast_path.py -q` **9 passed**，`../.venv/bin/python -m pytest tests/test_tutoring_retrieval.py tests/test_openapi_alignment.py -q` **36 passed**。契约：未修改 OpenAPI、schemas 或 Backend/Client API。
- `2026-06-14` `tutoring 检索 catalog_id 接入 + Guard 空术语放行 + 启动建集合`：**通过**。三问题根因：①课程知识在 Qdrant 按 `catalog_id` 入库，但 Backend 把 offering 的 `course_id`（如 `5a113a6fd12143da`）当检索键下发，按 `course_id` 过滤恒 0 命中；②`tutoring_response_critic._is_relevant` 在无任何可对齐术语（检索空+画像空+策略空）时恒判 `off_topic`，把 ReAct 有效回答清零，前端只剩规则套话；③`main.py` 启动仅 `build_qdrant_store()` 构造客户端包装、未真正建集合，`user_memory_v1_1024` 不存在导致检索 404。修复：`schemas/tutoring.py` 新增可选 `catalog_id`；`memory/tutoring_retrieval.py`、`agents/tutoring_react_flow.py` 课程知识检索改用 `catalog_id or course_id`；`backend/app/api/v1/tutoring.py` 下发已解析的 `offering.catalog_id`；`agents/tutoring_response_critic.py` 无可对齐术语时不再判 off_topic（grounding 仅在有术语时生效）；`main.py` 启动 `await ensure_collection_exists` 两个集合。契约：经用户授权同步 `docs/20-agent-api/Agent-Service.openapi.json` 的 `TutoringChatRequest.catalog_id`，未动 Client API OpenAPI。验证：`uv run pytest tests/test_tutoring_response_critic.py tests/test_tutoring_retrieval.py tests/test_openapi_alignment.py -q` 新增 2 条用例 + 对齐全过；全量 `uv run pytest -q` **465 passed, 9 failed**（9 条均为既存环境问题：缺 `pytest_asyncio` 致 6 条 `test_aichat_hybrid_retrieval` 异步用例无法运行、3 条 `test_assessment_difficulty_balancer` 为未触及的 assessment 规则逻辑，与本次改动无关）。真实数据复验：offering 键 `5a113a6fd12143da` search **0 命中**、catalog 键 `ac4843d2f7614174` search **3 命中**；`curl /collections` 现含 `user_memory_v1_1024`。剩余：命中 chunk 仍含 OCR 乱码，属既知素材质量问题，另行处理。
- `2026-06-14` `tutoring DeepSeek thinking tool-call compatibility`：**通过**。根因：AgentScope `ReActAgent` 在传入 `structured_model` 时会注册 `generate_response` 并强制 `tool_choice="required"`；DeepSeek thinking mode 官方文档支持 tool calls，但示例不传 `tool_choice`，当前真实请求报 `Thinking mode does not support this tool_choice`。修复：`agents/tutoring_react.py` 对 DeepSeek thinking/reasoning-like 模型（`deepseek-v4*` / `reasoner` / `r1` / `thinking`）跳过 `structured_model`，让普通 ReAct tool calls 自然执行，输出文本继续走既有 `parse_tutoring_model_response()` 解析；普通模型仍保留 structured metadata 路径。同步修复 `memory/tutoring_retrieval.py`：用户记忆 Qdrant collection 缺失/404 时只清空 `user_memory_facts` 并记录 warning，不再阻断课程知识检索。验证：`../.venv/bin/python -m pytest tests/test_tutoring_react.py tests/test_tutoring_react_flow.py tests/test_tutoring_retrieval.py tests/test_tutoring_agent.py tests/test_tutoring_fast_path.py -q -p no:cacheprovider` **47 passed**。契约说明：未修改 OpenAPI、schemas 或 Backend 调用契约。
- `2026-06-11` `KG residual TOC/index filter patch`：**通过**。按补点决策中的 `filter_review_first` 结果，增强 `memory/kg_body_grounding.py` 过滤规则，新增英文索引页码串、单行稀疏点线页码、`索引/目录 + 点线/页码` 噪声识别。TDD 覆盖 `expression ...，52，200` 英文索引、`...30 1.5.1 文件复制 ...31` 单行点线目录、`252 索引 ...` 尾部索引残留 fallback 到正文候选。真实 C 语言样本复验 `/tmp/kg-resource-probe/c-language-route-a-grounding-filtered-expanded-best-v3.json`：原 7 个 `residual_toc_or_index_noise` 节点均换成非索引候选，整体 `>=0.70` 仍为 `57/116=49.14%`。结论：过滤漏网已清理，但通过率没有提升到成功线，下一步仍是正文补点/聚类。验证：`./.venv/bin/pytest tests/test_kg_body_grounding.py tests/test_kg_body_grounding_tool.py tests/test_vector_store.py -q` **20 passed**。
- `2026-06-10` `KG body grounding filter + query expansion probe`：**阶段通过，整体未达线**。增强 `memory/kg_body_grounding.py` 正文候选过滤，排除点线页码目录、附录目录/索引等噪声；`tools/kg_body_grounding.py` 新增 `--query-expansion`，可选使用 `node.name` + `chapter/node_name/相邻节点名` 双 query 检索并取最高正文候选，默认行为不变。真实 C 语言样本 catalog `b2444963f0e54587` / KG `116` 节点重跑：baseline `22/116=18.97%`；仅过滤后 `18/116=15.52%`（纠正目录误判但不提分）；过滤 + query 扩展取最优后 `57/116=49.14%`，其中 baseline `0.65-0.70` 的 49 个边缘节点有 23 个过线，无已支持节点回退。结论：第 1、2 步有效但仍未达到固定成功线 `>=70%`，下一步应进入正文补点 / 正文聚类，而不是继续调阈值。验证：`./.venv/bin/pytest tests/test_kg_body_grounding.py tests/test_kg_body_grounding_tool.py tests/test_vector_store.py -q` **19 passed**；真实输出 `/tmp/kg-resource-probe/c-language-route-a-grounding-filtered.json`、`/tmp/kg-resource-probe/c-language-route-a-grounding-filtered-expanded-best.json`。
