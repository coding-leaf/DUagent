...
class ChapterProgressItem(BaseModel):
    ...
    completion_rate: float = Field(..., ge=0.0, le=100.0, description="完成率 0-100")
...
class QuizResultItem(BaseModel):
    ...
    score: float = Field(..., ge=0.0, le=100.0, description="正确率")
    created_at: datetime | None = Field(None, description="完成时间")
...
class EvaluationGenerateRequest(BaseModel):
    ...
    student_profile: dict[str, Any] | None = Field(None, description="用户基础画像")
