...
def test_quiz_diagnose_api_success() -> None:
    ...
    mock_response.text = (
        '{"diagnosis": {'
        '"summary": "本次作答有一些失误。",'
        '"score_analysis": "本次得分为 0 分。",'
        '"wrong_points_analysis": "未掌握基础加法概念。",'
        '"suggestions": ["建议加强基础加法练习。"]'
        '}}'
    )
    ...
    with patch("agent_service_v2.api.evaluation.build_chat_model_from_settings", return_value=mock_model):
        ...
        assert data["diagnosis"]["summary"] == "本次作答有一些失误。"
        assert data["diagnosis"]["suggestions"] == ["建议加强基础加法练习。"]
        prompt = mock_model.await_args.args[0][0].content[0].text
        assert "你是 EDUagent 的学习诊断助手。" in prompt
        assert "所有字符串值必须使用简体中文" in prompt
