...
- **高优先级：tutoring 知识点元数据字段不一致，导致知识点不会完整落库**
  - 审查发现 Agent SSE 事件发送 `knowledge_points`，Backend SSE 代理读取的是 `points`
  - 这会导致 tutoring 链路虽然能返回内容，但 `knowledge_points` 元数据实际无法稳定保存
  - 该问题会阻断“知识点入库/后续利用”的完整链路，应优先修复并回归验证
- **中优先级：`resources/generate` 在 Agent 立即失败时仍返回 `202 accepted`**
  - 当前实现里，若 Agent 调用阶段立即失败，Backend 已将 `AsyncTask` 置为 `failed`
  - 但接口响应可能仍返回 `202`，会让前端误以为任务已成功入队
  - 该问题不阻断主链，但建议在当前阶段顺手收口，避免异步语义混乱
