...
* **验证码获取与提交：** 登录与注册页面接入 `GET /auth/captcha` 获取图形验证码及关联 `captcha_token`；表单提交时，必须附带验证码输入值 `captcha_code` 及 token。
...
* **角色重定向保护：** 页面加载或登录后调用 `GET /users/me` 将获取到的 `UserInfo`（包含真实 `role`）存入全局 `AuthContext`。**移除根据用户名硬编码的逻辑**（如 `username === 'teacher'`），严格根据 `response.data.user.role` 或全局上下文角色重定向到 `/teacher`、`/dashboard` 或 `/admin`。
...
* **数据恢复与更新：**
  ...
  * **支持 URL 深链：** 所有的核心业务页面路由（如 Dashboard、LearningPath、Quiz、ResourceDetail、TeacherStudentReport 等）均优先识别 URL 中的查询参数（如 `?course_id=...&student_id=...&resource_id=...`），并将该参数同步更新到全局 Context 中。
  * 在 `TopNavBar` 中渲染课程切换下拉菜单，切换后更新 URL 并触发关联页面重新加载。
...
* **教学班级监控：** 
  ...
  * `/api/v1/course/{courseId}/students` -> `/teaching/classes/{class_id}/students` (学生名单)；
...
* **教师监控与警告屏蔽：** 隐藏 `TeacherConsole.jsx` 里的 AI 洞察模块（平均活跃时间、覆盖率）。学生列表只展示现有 `StudentSummary` 内的基础字段，隐藏无法从 API 获得的伪学情分析指标。
* **无接口按钮禁用：** 在 `TeacherStudentReport.jsx` 及其他页面上，对类似“发送反馈”、“导出报告”等暂无后端接口支撑的按钮，进行**禁用 (disabled) 或隐藏**处理。
...
### 7. Playwright E2E 测试环境配置
* **运行环境：** Playwright 的端到端测试用例必须配置运行在 **真实的 Backend 联调环境** 下（关闭 Mock 模式）。
* **测试凭证：** 使用数据库预置的真实测试账号（如学生账号、教师账号、管理员账号）和预置测试课程（数据结构与算法）进行测试，不应依赖 Mock API 数据。
* **用例覆盖：**
  1. 学生真实登录 -> 自动进入课程资源库。
