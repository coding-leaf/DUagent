...
async def test_generate_catalog_chunks_uses_agent_v2_and_creates_version():
    ...
    with patch(
        "app.services.kg_generation.kg_agent_client.post_json",
        new_callable=AsyncMock,
    ) as mock_post:
...
def test_catalog_kg_agent_timeout_matches_background_generation_budget():
    assert kg_agent_client.timeout == 300.0
