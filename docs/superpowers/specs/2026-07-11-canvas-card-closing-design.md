...
### 1. 状态管理扩展 (Frontend Context)
在 `frontend/src/context/ChatContext.jsx` 中，管理本地会话的已关闭卡片 ID：
...
#### `WorkspaceTabs.jsx`
- **标签名字与图标映射**：
  为所有 AgentCard 插件映射易读的中文标题与特定 Lucide 图标：
...
## 影响评估
- **纯前端修改**：由于关闭行为只是视觉上在前端对 Artifact Tabs 进行过滤渲染，不影响后台数据同步，所以**不涉及后端接口漂移**。
