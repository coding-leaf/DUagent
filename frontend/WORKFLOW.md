...
### 2026-06-18 (后端 catalogs.py 基础 Material/Catalog 服务提取)
- **改了什么文件**: `backend/app/api/v1/catalogs.py`, `backend/app/services/catalog_service.py`, `backend/app/services/catalog_material_service.py`, `backend/tests/test_catalog_service.py`, `backend/tests/test_catalog_material_service.py`。
- **核心改动**: 按 catalogs 模块化重构计划 Task 2，新增类风格 `CatalogMaterialService(db)` 与 material helper，迁移文件名校验、material 目录删除包装、资料新增后的状态计算；在 `CatalogService` 增加 `list_ready_catalogs()` 并让公开 `GET /course-catalogs` 委托 service。
- **测试结果**: RED: `tests/test_catalog_material_service.py tests/test_catalog_service.py::test_list_ready_catalogs_filters_deleted_and_orders_by_title` 因缺少 `app.services.catalog_material_service` 失败；GREEN: `tests/test_catalog_service.py` 2/2 passed；回归: `tests/test_catalog_material_service.py tests/test_catalog_service.py tests/test_course_catalogs.py tests/test_course_catalog_ingestion.py::test_admin_upload_catalog_material tests/test_course_catalog_ingestion.py::test_upload_removes_material_dir_for_empty_file` 18/18 passed；语法检查: `PYTHONPYCACHEPREFIX=/tmp/eduagent_pycache ../.venv/bin/python -m py_compile app/api/v1/catalogs.py app/services/catalog_service.py app/services/catalog_material_service.py` 通过。MySQL 回归使用非生产库 `catalog_task1_presenters_test`。
- **是否有接口漂移**: 无。公开 ready catalog 响应包装、上传错误码、文件名校验错误码和状态转换语义未改变。
- **代码审查结果**: `CatalogMaterialService` 保持类风格；文件系统删除单测 patch `app.services.catalog_material_service.shutil.rmtree`，未触碰真实 storage；路由仅替换 helper 调用和 ready catalog 查询委托，没有移动上传 DB 写入流程。

### 2026-06-18 (后端 catalogs.py 资料 CRUD 服务方法提取)
- **改了什么文件**: `backend/app/api/v1/catalogs.py`, `backend/app/services/catalog_material_service.py`, `backend/tests/test_catalog_material_service.py`, `backend/tests/test_course_catalog_ingestion.py`。
- **核心改动**: 按 catalogs 模块化重构计划 Task 3，将 material list / external create / delete 逻辑下沉到 `CatalogMaterialService`。Route 保留鉴权、Catalog 获取、响应包装和原有 delete commit 边界。
...
### 2026-06-18 (后端 catalogs.py 知识图谱任务服务提取)
- **改了什么文件**: `backend/app/api/v1/catalogs.py`, `backend/app/services/catalog_kg_service.py`, `backend/tests/test_admin_catalog_kg_generation.py`。
- **核心改动**: 按 catalogs 模块化重构计划 Task 5，新增类风格 `CatalogKGService(db)` 承载 catalog KG 状态查询、重复任务检查、知识库 ready/empty 校验、host course 复用/创建与任务创建；新增模块级 `run_catalog_kg_generation_background(task_id)` 承载后台 KG 生成执行。Route 仅保留鉴权、service 调用、background task 注册和响应包装。
- **测试结果**: RED: `tests/test_admin_catalog_kg_generation.py::test_catalog_kg_generation_without_offering_returns_202_not_40915` 因缺少 `app.services.catalog_kg_service` 失败；GREEN: 同测试 1/1 passed；修复 offering 优先复用回归后，`tests/test_admin_catalog_kg_generation.py::test_admin_get_catalog_kg_status_returns_empty_summary_with_course_id tests/test_admin_catalog_kg_generation.py::test_admin_catalog_kg_generation_creates_task_and_background_graph` 2/2 passed；回归: `tests/test_admin_catalog_kg_generation.py tests/test_course_knowledge_graph_versions.py` 21 passed, 1 skipped；语法检查: `PYTHONPYCACHEPREFIX=/tmp/eduagent_pycache ../.venv/bin/python -m py_compile app/api/v1/catalogs.py app/services/catalog_kg_service.py` 通过。MySQL 回归使用非生产库 `admin_catalog_kg_generation_task5`。
- **是否有接口漂移**: 无。保留 `task_type="kg_generation"`、响应字段、错误码、Agent 生成调用入参和原有 offering course 优先复用语义。
- **代码审查结果**: `CatalogKGService` 保持 request-scoped 类风格；后台 runner 为 service 模块级函数并自行创建 `async_session_factory()` session；host course 路径保留 `with_for_update()` 和 `IntegrityError` 并发兜底；测试 patch 路径已更新为 `app.services.catalog_kg_service.generate_knowledge_graph_version`。

### 2026-06-18 (后端 catalogs.py 资源生成服务提取)
- **改了什么文件**: `backend/app/api/v1/catalogs.py`, `backend/app/services/catalog_resource_generation_service.py`, `backend/tests/test_admin_catalog_resource_generation.py`。
- **核心改动**: 按 catalogs 模块化重构计划 Task 6，新增类风格 `CatalogResourceGenerationService(db)` 承载 catalog resource 列表聚合、admin 资源软删除、显式 metadata 资源生成、KG 节点 fanout 父子任务创建和 Agent 调用。Route 仅保留鉴权、Catalog 获取、webhook URL 计算、service 调用和响应包装。
- **测试结果**: RED: `tests/test_admin_catalog_resource_generation.py::test_admin_catalog_generation_without_metadata_fails_parent_when_no_active_kg` 因缺少 `app.services.catalog_resource_generation_service` 失败；GREEN: 同测试 1/1 passed；直接回归: `tests/test_admin_catalog_resource_generation.py tests/test_kg_resource_targets.py` 32/32 passed；语法检查: `PYTHONPYCACHEPREFIX=/tmp/eduagent_pycache ../.venv/bin/python -m py_compile app/api/v1/catalogs.py app/services/catalog_resource_generation_service.py` 通过。计划中的 `tests/test_node_resources.py` 在独立 MySQL 库 `node_resources_task6` 下仍有既有失败（1 passed, 1 failed，失败断言为 `1 check(s) FAIL`），该测试不经过本次迁移的 catalog resource service 路径。
- **是否有接口漂移**: 无。保留 `task_type="resource_generation"`、Agent 路径 `/agent/v1/resources/generate`、父/子任务 result 字段、错误码和列表/删除响应格式。
- **代码审查结果**: Agent mock 路径已更新为 `app.services.catalog_resource_generation_service.agent_client.post_json`；service 不引入后台 runner；资源生成仍在请求内按原逻辑创建任务并调用 Agent；未修改 webhook 聚合逻辑。

### 2026-06-18 (后端 catalogs.py 题库生成服务提取)
- **改了什么文件**: `backend/app/api/v1/catalogs.py`, `backend/app/services/catalog_quiz_generation_service.py`, `backend/tests/test_admin_catalog_resource_generation.py`。
- **核心改动**: 按 catalogs 模块化重构计划 Task 7，新增类风格 `CatalogQuizGenerationService(db)` 承载 quiz 父子任务创建，新增模块级 `run_quiz_generation_background()` 与 `generate_quiz_for_child()` 承载后台并发执行、Agent 出题调用、skeleton fallback 拒绝、题目落库和父任务聚合。Route 仅保留 Catalog 获取、service 调用、后台任务调度和响应包装。
...
### 2026-06-18 (修复节点资源基线回归)
- **改了什么文件**: `backend/app/api/v1/learning_path.py`, `src/api/__tests__/client.test.js`, `WORKFLOW.md`, `docs/requirements-coverage.md`。
- **核心改动**: 修复 `GET /api/v1/learning-path/nodes/{node_id}/resources` 在旧课程直接挂 `CourseKnowledgeGraph(course_id=course_id)` 时无法解析 active KG chapter 的问题。接口优先沿用 catalog host course KG；当该链路不存在或未命中 KG 时，fallback 到当前 `course_id` 的 active KG，从而恢复 `chapter_materials` 查询。另修复前端检查时暴露的 `client.test.js` 漏导入 `afterEach` 的 lint 问题。
- **测试结果**: RED: `TEST_DATABASE_URL=mysql+aiomysql://root:123456@127.0.0.1:3306/node_resources_task6?charset=utf8mb4 ../.venv/bin/python -m pytest tests/test_node_resources.py -q -p no:cacheprovider` 初始为 1 passed, 1 failed，失败为 `tests/test_node_resources.py::test` 中 `1 check(s) FAIL`；GREEN: 同命令 2/2 passed；语法检查: `PYTHONPYCACHEPREFIX=/tmp/eduagent_pycache python3 -m py_compile app/api/v1/learning_path.py` 通过；最终 catalogs 组合回归 `tests/test_catalog_presenters.py tests/test_catalog_material_service.py tests/test_catalog_service.py tests/test_course_catalogs.py tests/test_course_catalog_ingestion.py tests/test_admin_catalog_kg_generation.py tests/test_course_knowledge_graph_versions.py tests/test_admin_catalog_resource_generation.py tests/test_kg_resource_targets.py tests/test_node_resources.py` 在 MySQL 库 `catalog_full_task8` 下 79 passed, 2 skipped，仅有既有 aiomysql event loop close 资源警告；前端 `npm run test:unit -- src/api/__tests__/client.test.js` 7/7 passed，`npm run lint` 通过，`npm run build` 通过（仍有既有 chunk size warning）。
- **是否有接口漂移**: 无。响应字段、状态码和资源分组 key 均未改变。

### 2026-06-19 (后端 learning_path.py 分层重构)
- **改了什么文件**: `backend/app/api/v1/learning_path.py`, `backend/app/services/learning_path_service.py`, `backend/app/services/learning_path_refresh_service.py`, `backend/app/services/node_resource_service.py`, `backend/tests/test_learning_path_service.py`, `backend/tests/test_learning_path_refresh_service.py`, `backend/tests/test_node_resource_service.py`, `backend/tests/test_learning_path_realtime.py`, `backend/tests/test_learning_path_fallback.py`, `backend/tests/test_node_resources.py`, `backend/tests/test_refresh_async.py`, `backend/tests/test_lock_async.py`, `WORKFLOW.md`, `docs/requirements-coverage.md`。
- **核心改动**: 按 `Router -> Service -> DB` 分层将 500 行级学习路径胖路由拆分为读取服务、刷新服务和节点资源服务。`LearningPathService` 承载快照读取、实时进度合并和 KG fallback；`LearningPathRefreshService` 承载 Agent payload、AsyncTask 创建和独立 session 后台 runner；`NodeResourceService` 承载权限、资源 scope、KG/LP 节点解析和资源聚合。路由缩减至约 96 行，仅保留依赖注入、学生选课 guard、响应包装、后台调度和明确的 `# commit point: task persisted before background dispatch`。
