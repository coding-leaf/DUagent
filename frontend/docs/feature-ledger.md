...
| `AIChat.jsx` | 查看会话、历史消息、SSE 对话；左右侧边栏支持桌面折叠及移动端抽屉收缩，支持遮罩交互与会话联动 | `GET /tutoring/conversations`、`GET /tutoring/conversations/{id}`、`POST /tutoring/chat` | ✅ 已可操作 | 已兼容对象型 `knowledge_points`；已完成侧边栏响应式与桌面折叠功能升级。 |
