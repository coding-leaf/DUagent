...
当前已经实现“管理员在资源库维度生成学习资源并做表面软删除”的链路：

1. Admin 在 `CourseCatalogDrawer.jsx` 查看资料、知识库状态和生成资源列表。
2. 抽屉通过 `adminService.getCourseCatalogResources()` 调用 `GET /admin/course-catalogs/{catalog_id}/resources`，聚合当前绑定该资源库的教学班下未软删除资源。
3. Admin 填写章节、知识点并选择资源类型，前端调用 `POST /admin/course-catalogs/{catalog_id}/resources/generations`。
4. Backend 创建 `resource_generation` AsyncTask，Agent 返回资源后通过 webhook fan-out 到当前绑定的教学班 `resources.course_id`。
5. 前端通过 `taskService.getTaskStatus()` 独立轮询生成任务；终态后刷新生成资源列表。
6. Admin 可调用 `DELETE /admin/resources/{resource_id}` 软删除生成资源，可调用 `DELETE /admin/course-catalogs/{catalog_id}/materials/{material_id}` 软删除资料。
7. 软删除只影响可见列表和资料删除后的 `knowledge_status=dirty`，不删除上传文件、Qdrant chunks 或 Agent 产物。

本链路的边界：

- 自动化测试已覆盖 Admin 生成任务轮询、生成资源列表刷新、资源软删除、资料软删除、教师端无生成入口。
- 部署级 live smoke 仍建议执行，用于确认真实 Agent 资源返回、Backend fan-out 和前端刷新在多服务环境下一致。
...
| CourseCatalog 管理 | `AdminConsole.jsx`、`CourseCatalogDrawer.jsx`、`adminService` | `/admin/course-catalogs`、`/admin/course-catalogs/{catalog_id}` | `catalogs.py` | 无直接 Agent 依赖 | `catalog.py` | `test_course_catalogs.py`、Admin 入库/生成 UI E2E | 已闭环 | Admin UI 已接 catalog 管理；非 Admin 仅通过 `/course-catalogs` 选择 ready 资源库。 |
| CourseCatalog 资料上传 / 登记 / 软删除 | `CourseCatalogDrawer.jsx`、`adminService.uploadCourseCatalogMaterial`、`adminService.createCourseCatalogMaterial`、`adminService.deleteCourseCatalogMaterial` | `/admin/course-catalogs/{catalog_id}/materials/upload`、`/admin/course-catalogs/{catalog_id}/materials`、`GET /materials`、`DELETE /admin/course-catalogs/{catalog_id}/materials/{material_id}` | `catalogs.py` | 无直接 Agent 依赖 | `CourseCatalogMaterial` | `test_course_catalog_ingestion.py`、`test_admin_catalog_resource_generation.py`、Admin 入库/生成 UI E2E | 已闭环 | 管理员界面已支持选择 `txt/md/pdf` 上传；软删除不删除文件或 chunks；不要提交上传文件或存储产物。 |
...
| Admin 资源库级资源生成 / 资源软删除 | `CourseCatalogDrawer.jsx`、`adminService.getCourseCatalogResources`、`startCourseCatalogResourceGeneration`、`deleteResource` | `/admin/course-catalogs/{catalog_id}/resources`、`/admin/course-catalogs/{catalog_id}/resources/generations`、`/admin/resources/{resource_id}`、`/tasks/{task_id}` | `catalogs.py`、`tasks.py`、`webhooks.py`、`agent_client.py` | `api/v1/resources.py`、`agents/resources_workflow.py`、`agents/resources.py` | `others.AsyncTask`、`others.Resource`、`catalog.py` | `test_admin_catalog_resource_generation.py`、`test_resources_async.py`、Admin 生成/软删除 UI E2E | 已闭环 | Admin-only 资源库级生成已接 UI；按当前绑定教学班 fan-out；生成后才绑定的班级不自动回补；部署级 live smoke 仍建议做。 |
| 历史资源生成 | 当前无教师/学生正式前端入口；曾有 orphan service 已删除 | `/resources/generate` | `resources.py`、`course_catalog_gate.py`、`agent_client.py` | `api/v1/resources.py`、`agents/resources_workflow.py`、`agents/resources.py` | `others.AsyncTask`、`others.Resource`、`catalog.py` | `test_resources_async.py`、`test_course_catalog_ready_gate.py`、Agent `test_resources_workflow.py` | 前端不接入 / 历史接口 | 当前前端契约作废 / 不接入，教师端不提供生成资源入口；保留为后端内部能力或废弃候选背景。 |
...
| AsyncTask 查询 | `CourseCatalogDrawer.jsx`、`taskService` | `/tasks/{task_id}` | `tasks.py` | 无直接 Agent 依赖 | `others.AsyncTask` | Admin 入库 UI E2E、Admin 生成/软删除 UI E2E、生成相关后端测试 | 已闭环 | 前端只在已接 UI 中使用；当前用于 Admin CourseCatalog 入库和资源库级生成轮询，不作为教师资源/Quiz 生成入口。 |
...
- Resources：`/resources`、`/resources/{id}`；Admin 资源库级生成/管理端点；`/resources/generate` 为前端作废 / 不接入的历史接口
...
| `catalogs.py` | `/agent/v1/resources/generate` | Admin 资源库级学习资源生成 | 真实 Agent 返回、Backend fan-out、空资源失败处理、生成后绑定班级不回补 |
| `resources.py` | `/agent/v1/resources/generate` | 历史教师资源生成 / 后端内部能力候选 | 当前不作为教师端下一步验收目标；如需恢复需重新定义生成产品链路 |
...
- Admin resource generation：`test_admin_catalog_resource_generation.py`
...
- `npm run test:e2e -- e2e/specs.spec.js -g "Admin course catalog resource generation|Teacher console does not expose"`：2/2 通过。

## 覆盖结论

`docs/project-direction.md` 覆盖了当前项目的主结构和方向，但不能替代本审计表。按当前证据，项目覆盖状态如下：

- 主链路已闭环：Auth、课程、CourseCatalog 管理、管理员资料上传、CourseCatalog 资料入库和向量化、Admin 资源库级资源生成、资料/资源软删除、资源列表/详情、基础 Quiz、AI Chat、教师基础学情、AsyncTask 查询。
- 教师/学生侧历史生成链路已降级为前端不接入 / 历史接口：`/resources/generate` 和 Quiz 生成虽已有 ready gate、契约和后端测试背景，但不再作为教师端或学生端前端主线。
...
- 高风险下一步：Admin CourseCatalog 入库和资源生成部署级验收、教师绑定 ready CourseCatalog 创建教学班验收、学生 / 教师消费入库与生成内容的产品口径、LearningPath/KG ready gate 设计。

## 下一轮审计动作

1. 做 Admin CourseCatalog 上传 / 入库 / 资源生成部署级验收，记录真实 Backend、Agent Service、MySQL、Qdrant/storage/provider 配置下的任务、知识库状态和生成资源列表证据。
