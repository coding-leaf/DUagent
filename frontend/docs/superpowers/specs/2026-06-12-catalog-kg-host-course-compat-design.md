...
该方案目标是先解除管理员侧 409 阻塞，同时把后续完整迁移拆成独立第二阶段。
...
- 不把宿主课暴露给教师/学生前端使用
- 为后续“KG 真正迁移到 `catalog_id`”保留清晰退出路径
...
如果后续不推进第二阶段，系统会长期保留“资源库中心资源 + 课程中心 KG”的双轨模型。

缓解：

- 在 feature ledger 和 workflow 中明确标记为阶段性兼容层

## Phase 2 Direction

第二阶段目标：

- `CourseKnowledgeGraph` 真正归属 `catalog_id`
- `get_active_knowledge_graph` 支持按 `catalog_id` 读取
- LearningPath、节点资源、评估探针等逐步从 `course_id` 依赖迁出
- 删除宿主课兼容层和相关字段

在第二阶段完成前，第一阶段方案只应被视为止血层，不是最终架构。
