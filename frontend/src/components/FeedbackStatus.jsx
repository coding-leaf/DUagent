export default function FeedbackStatus({ status, title, description, onRetry }) {
  if (status === 'loading') {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center">
        ...
        <h4 className="text-lg font-bold text-on-surface mb-2">{title || '加载中...'}</h4>
  ...
  if (status === 'empty') {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center border border-dashed border-outline-variant rounded-2xl bg-surface-container-lowest">
        <span className="material-symbols-outlined text-outline text-5xl mb-4">
          folder_open
        </span>
        <h4 className="text-lg font-bold text-on-surface mb-2">{title || '暂无数据'}</h4>
  ...
  if (status === 'error') {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center border border-error-container/30 rounded-2xl bg-error-container/5">
