...
export default function LearningEffects() {
  ...
  const masteryDistribution = useMemo(() => {
    const groups = [
      ...
      { key: 'unassessed_default_pass', label: '未测评/默认通过', count: 0, color: 'bg-slate-400' },
    ];
    ...
    nodeRows.forEach((row) => {
      ...
      } else if (row.assessment_state === 'unassessed_default_pass') {
        byKey.unassessed_default_pass.count += 1;
